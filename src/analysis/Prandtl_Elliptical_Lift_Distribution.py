...
class EllipticalLiftDistribution:
    def __init__(
        ...
        logging.info(
            f"Parameters: span={span} in, lift_coefficient={lift_coefficient}, "
            f"rho={rho}, velocity={velocity} ft/s, root_chord={root_chord} in, "
            f"tip_chord={tip_chord} in"
        )
    ...
    def log_results(self, positions, lifts_chords_loads_shear_moment):
        for pos, (lift, chord, load, shear_force, bending_moment) in zip(positions, lifts_chords_loads_shear_moment):
            logging.info(
                f"Lift per unit span at {pos:.2f} in: {lift:.2f} lb/in, "
                f"Chord length: {chord:.2f} in, Load: {load:.2f} lb, "
                f"Shear force: {shear_force:.2f} lb, Bending moment: {bending_moment:.2f} lb-in"
            )
    ...
    def size_spar(self, bending_moments, yield_strength):
        ...
        b = 0.5  # Initial guess for width in inches
    ...
    def calculate_torsional_load(self, positions, moment_arm_lift_array, moment_arm_drag_array):
        ...
        for i, pos in enumerate(positions):
            ...
            logging.info(
                f"Position {pos:.2f} in, Lift {lift:.2f} lb, Drag {drag:.2f} lb, Torque {torques[i]:.2f} lb-in"
            )
    ...
    def plot_torsional_load(self, positions, torques, filename="torsional_load.png"):
        ...
        plt.savefig(os.path.join(self.output_folder, filename))
        plt.show()
        logging.info("Plotted and saved the torsional load distribution")
...
lifts_chords_loads_shear_moment = [
    (lift, chord, load, shear_force, bending_moment)
    for (lift, chord), load, shear_force, bending_moment in zip(lifts_and_chords, loads, shear_forces, bending_moments)
]

# Plot distributions
distribution.plot_distribution(
    positions,
    [lift for lift, _, _, _, _ in lifts_chords_loads_shear_moment],
...
torques, total_torsional_load = distribution.calculate_torsional_load(
    positions, moment_arm_lift_array, moment_arm_drag_array
)
