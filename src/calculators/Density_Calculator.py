...
logging.basicConfig(
    filename="log.txt", filemode="a", level=logging.DEBUG, format="%(asctime)s - %(levelname)s - %(message)s"
)
...
def calculate_air_density(altitude, humidity, barometric_pressure):
    ...
    logging.info(
        f"Air density at {temperature:.2f} K, {humidity}% humidity, and {barometric_pressure} Pa pressure "
        f"is {air_density:.2f} kg/m^3)"
    )
    return air_density
...
def main_density(altitude, humidity, barometric_pressure):
    ...
    logging.info(
        f"Air density at {altitude} meters, {humidity}% humidity, and "
        f"{barometric_pressure} Pa pressure is {density:.2f} kg/m^3"
    )
    return density
