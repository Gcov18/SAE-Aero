...
try:
    while True:
       ...
       # print("Load cell: {:.0f}, Distance: {:.0f}".format(loadCellValue, tofValue))
        ...
        file2write=open("Data_Test_SPAR_test.txt",'a')
        ...
        Load =  ((loadCellValue + 11650 +200 - 8900)/ 13.334 * 1.5)
        Tof = (tofValue - 32 )/.8#(tofValue - 77) / .78
        file2write.write(str(Load) + " " + str(Tof) + "\n")
        file2write.flush()
        file2write.close()
        
        print("Load: {:.0f}, Distance: {:.0f}".format(Load, Tof))
        # append to the list 
        lst_load += [Load]
        lst_tof += [Tof]
