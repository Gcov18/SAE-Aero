...
class EllipticalLiftDistribution:
    def __init__(
        ...
        logging.info(
            f"Parameters: span={span} in, lift_coefficient={lift_coefficient}, rho={rho}, "
            f"velocity={velocity} ft/s, root_chord={root_chord} in, tip_chord={tip_chord} in"
        )
    ...
    def lift_at_position(self, y_position):
        ...
        lift = (
            (4 * self.lift_coefficient * self.rho * self.velocity**2 * chord_length)
            / (np.pi * self.span)
            * np.sqrt(1 - (y_position / b) ** 2)
        )
    ...
    def calculate_distributions(self, positions):
        ...
        for i in range(len(positions) - 1):
            ...
            logging.info(
                f"Position {positions[i]:.2f} in, Lift {lifts_and_chords[i][0]:.2f} lb/in, "
                f"Delta Pos {delta_pos:.2f} in, Load {loads[i]:.2f} lb, "
                f"Torsional Load {torsional_loads[i]:.2f} lb-in"
            )
        ...
        logging.info(
            f"Position {positions[-1]:.2f} in, Lift {lifts_and_chords[-1][0]:.2f} lb/in, "
            f"Delta Pos {positions[-1] - positions[-2]:.2f} in, Load {loads[-1]:.2f} lb, "
            f"Torsional Load {torsional_loads[-1]:.2f} lb-in"
        )
    ...
    def plot_distribution(self, positions, values, ylabel, title, filename, figure_num, color="b", fontweight="normal"):
        ...
        plt.ylabel(ylabel)
        plt.title(
            f"{title}\nVelocity: {self.velocity} ft/s, Lift Coefficient: {self.lift_coefficient}, "
            f"Air Density: {self.rho} slugs/ft³",
    ...
    def log_results(self, positions, lifts_chords_loads_shear_moment):
        for pos, (lift, chord, load, shear_force, bending_moment, torsional_load) in zip(
            positions, lifts_chords_loads_shear_moment
        ):
            logging.info(
                f"Lift per unit span at {pos:.2f} in: {lift:.2f} lb/in, Chord length: {chord:.2f} in, "
                f"Load: {load:.2f} lb, Shear force: {shear_force:.2f} lb, "
                f"Bending moment: {bending_moment:.2f} lb-in, Torsional load: {torsional_load:.2f} lb-in"
            )
    ...
    def size_spar(self, bending_moments, yield_strength):
        ...
        b = 0.25  # Initial guess for width in inches
...
if __name__ == "__main__":
    # Configure logging
    logging.basicConfig(
        filename="Wing_Loading_Log.log", level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
    )
    ...
    lifts_and_chords, loads, shear_forces, bending_moments, torsional_loads = distribution.calculate_distributions(
        positions
    )

    # Combine lift, chord, load, shear force, bending moment, and torsional load into a single list
    lifts_chords_loads_shear_moment = [
        (lift, chord, load, shear_force, bending_moment, torsional_load)
        for (lift, chord), load, shear_force, bending_moment, torsional_load in zip(
            lifts_and_chords, loads, shear_forces, bending_moments, torsional_loads
        )
    ]
    ...
    distribution.plot_distribution(
        positions,
        [lift for lift, _, _, _, _, _ in lifts_chords_loads_shear_moment],
        "Lift per Unit Span (lb/in)",
    ...
    distribution.plot_distribution(
        positions, loads, "Load (lb)", "Load Distribution", "load_distribution.png", figure_num=2, color="Orange"
    )
    distribution.plot_distribution(
        positions,
        shear_forces,
    ...
    distribution.plot_distribution(
        positions,
        torsional_loads,
        "Torsional Load (lb-in)",
        "Torsional Load Distribution",
        "torsional_load_distribution.png",
        figure_num=5,
        color="Blue",
    )
...
class Aileron:
    def __init__(
        self,
        root_chord_length_ft,
        tip_chord_length_ft,
        span_ft,
        hinge_line_position_ft1,
        hinge_line_position_ft2,
        lift_coefficient,
        rho_slug_ft3,
        velocity_ft_s,
        deflection_angle,
