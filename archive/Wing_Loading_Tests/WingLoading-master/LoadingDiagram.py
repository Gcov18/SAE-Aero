...
class LoadingDiagram:
    def __init__(
        ...
        Cls: list,  # [(cl1, cla1 ,x1),(cl2, cla2, x2)] x1< x <x2 gives cl1< cl <cl2 in a linear relation.
        # x>x2 or x<x1 , cl=cl2 and cl=cl1 respectively. all at alpha = 0
        Cm25s: list,  # [(cm1, cma1 ,x1),(cm2, cma2, x2)] x1< x <x2 gives cm1< cm <cm2 in a linear relation.
        # x>x2 or x<x1 , cm=cm2 and cm=cm1 respectively. all at alpha = 0
        Tcs: list,  # [(t/c,x1), (t/c,x2)]
        # c_l_alpha = 2*pi,    # change in cl per radian for now only a global average is assumed.
        # If not given the standard solution for thin airfoils is used
    ...
    def getCl(self, x1, x2):
        ...
        for i in range(len(self.cls)):
            if x1 < self.cls[i][2]:
                    ...
                    cl1 = (
                        (
                            ...
                            (
                                self.cls[i][1] * self.a
                                + self.cls[i][0]
                                - (self.cls[i - 1][1] * self.a + self.cls[i - 1][0])
                            )
                            /
                            #   x2         -    x1                  + (c_l_alpha_1    *alpha_1+ cl0_1)
                            (self.cls[i][2] - self.cls[i - 1][2])
                        )
                        * x1
                        + self.cls[i - 1][1] * self.a
                        + self.cls[i - 1][0]
                    )
                else:
                ...
                break
        if cl1 is None:
            cl1 = self.cls[-1][1] * self.a + self.cls[-1][0]
        for i in range(len(self.cls)):
            if x2 < self.cls[i][2]:
                if i > 0:
                    cl2 = (
                ...
                break
        if cl2 is None:
            cl2 = self.cls[-1][1] * self.a + self.cls[-1][0]
    ...
    def getThickness(self, x, cr):
        ...
        for i in range(len(self.Tcs)):
            if x < self.Tcs[i][1]:
                if i > 0:
                    tc = (self.Tcs[i][0] - self.Tcs[i - 1][0]) / (self.Tcs[i][1] - self.Tcs[i - 1][1]) * x + self.Tcs[
                        i - 1
                    ][0]
                else:
                    tc = self.Tcs[i - 1][0]
    ...
    def getCm(self, x1, x2):
        ...
        for i in range(len(self.cms)):
            if x1 < self.cms[i][2]:
                    ...
                    cm1 = (
                        (
                            ...
                            (
                                self.cms[i][1] * self.a
                                + self.cms[i][0]
                                - (self.cms[i - 1][1] * self.a + self.cms[i - 1][0])
                            )
                            /
                            #   x2         -    x1                  + (c_l_alpha_1    *alpha_1+ cl0_1)
                            (self.cms[i][2] - self.cms[i - 1][2])
                        )
                        * x1
                        + self.cms[i - 1][1] * self.a
                        + self.cms[i - 1][0]
                    )
                else:
                ...
                break
        if cm1 is None:
            cm1 = self.cms[-1][1] * self.a + self.cms[-1][0]
        for i in range(len(self.cms)):
            if x2 < self.cms[i][2]:
                if i > 0:
                    cm2 = (
                ...
                break
        if cm2 is None:
            cm2 = self.cms[-1][1] * self.a + self.cms[-1][0]
    ...
    def genDiagrams(self, V, rho, filename=None):
        ...
        plt.title(
            "Diagrams over half of the wing\n for V={} [m/s], n={} and fuel level={}".format(
                V, self.loadFactor, self.fuelLevel
            )
        )
        plt.xlim(xmin=0.0)
        if moments["Ms"][0] > 0:
        ...
        diagrams = {"Moment": moments, "Shear": shears, "Torque": torques}
        self.diagrams = diagrams
        return diagrams

    def tipDeflection(
        self, t, tskin=None
    ):  # determines the tip defelction given a spar thickness. Uses the last call to gen diagrams
        ...
        for i in range(len(self.__segments)):
            ...
            moment_of_inertia = 2 * ((1 / 12) * t * seg[7] ** 3)
            moment_of_inertia += 2 * tskin * 0.45 * seg[6] * (seg[7] / 2) ** 2 if tskin is not None else 0.0
            Is.append(moment_of_inertia)
            ...
            deltas.append(delta)
            stress.append((moments[i + 1] * seg[7] / 2) / moment_of_inertia)
    ...
    def getRequiredThicknessTwist(
        self, theta, tspar
    ):  # returns the required sheet thikness for a given twist. theta in deg
        t = 0.00001
        th = 0.0
        Tmax = 324 * (10**6)
        theta = radians(theta)
    ...
    def getRequiredThickness(
        self, delta, theta
    ):  # returns the spar and skin thickness required for the given deflection and twist. theta in deg
