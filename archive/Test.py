...
# Configure logging
logging.basicConfig(
    filename="Aileron_Loading_Log.log", level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
)
...
class Aileron:
    def __init__(
        ...
        logging.info(
            f"Aileron initialized with root_chord_length_in={root_chord_length_in}, "
            f"tip_chord_length_in={tip_chord_length_in}, span_in={span_in}, "
            f"lift_coefficient={lift_coefficient}, rho_slug_ft3={rho_slug_ft3}, "
            f"velocity_ft_s={velocity_ft_s}, deflection_angle={deflection_angle}"
        )
    ...
    def calculate_lift_force(self):
        ...
        logging.info(
            f"Calculated lift_force_lb={total_lift_force} with q={q}, "
            f"average_chord_length_in={average_chord_length_in}, "
            f"lift_per_unit_span={lift_per_unit_span}, "
            f"adjusted_lift_coefficient={adjusted_lift_coefficient}"
        )
    ...
    def calculate_torque(self, hinge_line_position_in):
        ...
        logging.info(
            f"Calculated torque_in_lb={torque_in_lb} with lift_force_lb={lift_force_lb}, moment_arm_in={moment_arm_in}"
        )
        return torque_in_lb

    def draw_wing_with_aileron(self, wing_root_chord_in, wing_tip_chord_in, wing_span_in):
        logging.info(
            f"Drawing wing with aileron with wing_root_chord_in={wing_root_chord_in}, "
            f"wing_tip_chord_in={wing_tip_chord_in}, wing_span_in={wing_span_in}"
        )
        fig, ax = plt.subplots()
        ...
        aileron_x_left = [19.18 + self.root_chord_length_in, 19.18, 15.22, 15.22 + self.tip_chord_length_in]
        aileron_y_left = [82 - 31.62, 82 - 31.62, 82, 82]
